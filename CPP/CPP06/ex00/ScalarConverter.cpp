...
/*   Updated: 2025/11/12 12:27:58 by hthant           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (pLiteral == "nan" || pLiteral == "-inf" || pLiteral == "+inf" || pLiteral == "inf" ||
			pLiteral == "nanf" || pLiteral == "-inff" || pLiteral == "+inff" || pLiteral == "inff");
}
...
void ScalarConverter::convert(const std::string &literal)
{
	...
	if (*end == '\0' && errno != ERANGE)
	{
		...
		if (dval >= std::numeric_limits<int>::min() && dval <= std::numeric_limits<int>::max())
			...
			std::cout << "int: impossible" << std::endl;
		if (dval >= std::numeric_limits<float>::min() && dval <= std::numeric_limits<float>::max())
